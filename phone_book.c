...
z:
    ...
    got(p.Gender);
    if(strcmp(p.Gender,"male")==0||strcmp(p.Gender,"female")==0||strcmp(p.Gender,"eunuch")==0)
    {
        ;
    }
    else
...
void listrecord()
{
    ...
    {
        printf("\n\n\n YOUR RECORD IS\n\n ");
        printf("\nName=%s\nAdress=%s\nFather name=%s\nMother name=%s\nMobile no=%ld\nGender=%s\nE-mail=%s\nhome_town=%s",p.name,p.address,p.father_name,p.mother_name,p.mble_no,p.Gender,p.mail,p.home_town);
...
void searchrecord()
{
    ...
    else
    {
        ...
        case '6':
            //deleterecord();
            printf("\nEnter Gender of person to search\n");
            got(Gender);
            while(fread(&p,sizeof(p),1,f)==1)
            {
                if(strcmp(p.Gender,Gender)==0)
                {
                    printf("\n\tDetail Information About %s",Gender);
                    printf("\nName:%s\naddress:%s\nFather name:%s\nMother name:%s\nMobile no:%ld\nGender:%s\nE-mail:%s\nhome_town:%s",p.name,p.address,p.father_name,p.mother_name,p.mble_no,p.Gender,p.mail,p.home_town);
                }
                else
                    printf("\nfile not found");
...
void deleterecord()
{
    ...
    else
    {
        ...
        got(name);  // Assume got() is a custom function to get the name
        ...
        if (flag == 1)
        {
            // Reopen original file in write mode to overwrite its contents
            f = fopen("project", "wb");
            ft = fopen("temp", "rb");

            if (f == NULL || ft == NULL)
            {
                printf("\nFile reopening error");
                if (f) fclose(f);
                if (ft) fclose(ft);
                remove("temp");  // Cleanup temporary file
                return;
            }
            ...
            fclose(f);
            fclose(ft);
            remove("temp");  // Delete temporary file
        ...
        else
        {
            ...
            remove("temp");  // Delete temporary file as no changes were made
        }
    }
...
void modifyrecord()
{
    ...
    else
    {
        ...
        case '6':
            //deleterecord();
            printf("\nEnter CONTACT'S Gender TO MODIFY:\n");
            got(Gender);
            while(fread(&p,sizeof(p),1,f)==1)
            {
